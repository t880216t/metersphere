 ...
 * </p>
 */
public class SSORealm extends BaseRealm {

    private Logger logger = LoggerFactory.getLogger(SSORealm.class);
    @Resource
    ...
    protected AuthenticationInfo doGetAuthenticationInfo(AuthenticationToken authenticationToken) throws AuthenticationException {
        ...
        return loginSSOMode(userId, password);
    }

    private AuthenticationInfo loginSSOMode(String userId, String password) {
        // userId 或 email 有一个相同就返回User
        String email = (String) SecurityUtils.getSubject().getSession().getAttribute("email");
        UserDTO user = userService.getLoginUser(userId, Arrays.asList(UserSource.SSO.name(), UserSource.LOCAL.name()));
        String msg;
        if (user == null) {
